...
BOT_TOKEN = "7946170487:AAEVjjJKEa5SHbvgqAlEhep1kHuccarzQxo"

# Admin password for /pass command
ADMIN_PASSWORD = "Rohan@1234"
...
async def bgmi(update: Update, context: CallbackContext):
    ...
    if user_id not in user_access:
        await update.message.reply_text("⚠️ You don't have access. Please pay ₹100 for 5 days access using /pay. CONTACT @Rohan2349")
        return
...
async def pay(update: Update, context: CallbackContext):
    await update.message.reply_text("Pay using UPI ID: rohan1003@fam")
...
async def handle_transaction_id(update: Update, context: CallbackContext):
    ...
    if transaction_id.strip():
        ...
        await update.message.reply_text("✅ Payment verified! Now you can use the bot.")
    else:
...
async def trial(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id="1866961136", text=admin_message)
